...
\begin{problems}
    ...
    \begin{sol}
        ...
        \[\sum_{i = 1}^{\lfloor \log_p(n)\rfloor} \left\lfloor\frac{n}{p^i}\right\rfloor \qh\]
    \end{sol}
...
\begin{problems}
    ...
    \begin{sol}
        ...
        \end{align*}
    \end{sol}
    \item Prove that the distinct equivalence classes in $\intmod$ are precisely $\bar 0, \bar 1, \bar 2, \ldots, \bar{n - 1}$ (use the Division Algorithm).
    \begin{sol}
        Clearly, $\bar 0, \bar 1, \bar 2, \ldots, \bar{n - 1} \in \intmod$. Let $\bar a \in \intmod$. By the Division Algorithm, there exists $q \in \z$ and $0 \leq r < n$ such that $a = nq + r$. Then $a \equiv r \bmod n$, so that $\bar a = \bar r$, where $r$ is any one of the aforementioned equivalence classes. Moreover, if $\bar a = \bar b$ where $0 \leq a, b < n$, it follows that $n \mid (a - b)$ so that $a - b = 0$, or $a = b$. Hence, the equivalence classes of $\intmod$ are precisely the set above.
    \end{sol}
    ...
    \end{sol}
    \item Prove that the square of the elements in $\intmod[4]$ are just $\bar 0$ and $\bar 1$.
    \begin{sol}
        \begin{align*}
    ...
    \begin{sol}
        ...
        Let $\bar a \in \units$. Then there exists $\bar b \in \units$ such that $\bar a \cdot \bar b = \bar 1$. In particular, $\bar a \cdot \bar b - \bar 1 = \bar 0$, or $n \mid (ab - 1)$. Then $nx + ab = 1$ for $x \in \z$, which shows that $(a, n) = 1$. Conversely, if $\bar a \in \units$ such that $(a, n) = 1$, then there exists $b, x \in \z$ such that $ab + xn = 1$, or $ab = 1 \bmod n$. It follows that $\bar b$ is the inverse of $\bar a \in \units$.
    \end{sol}
    \item Prove that if $\bar a, \bar b \in \units$, then $\bar a \cdot \bar b \in \units$.
    \begin{sol}
        It follows that there exist $\bar x, \bar y \in \units$ such that $\bar a \cdot \bar x = \bar b \cdot \bar y = \bar 1$. In particular:
        $$\bar 1 = \bar 1 \cdot \bar 1 = (\bar a \cdot \bar x)\cdot(\bar b \cdot \bar y) = (\bar a \cdot \bar b) \cdot (\bar x \cdot \bar y)$$
        So that the multiplicative inverse of $\bar a \cdot \bar b$ is $\bar x \cdot \bar y$.
    \end{sol}
    ...
    \end{sol}
    \item Conclude from the previous two exercises that $\units$ is the set of elements $\bar a$ of $\intmod$ with $(a, n) = 1$ and hence prove Proposition 4. Verify this directly in the case $n = 12$.
    \begin{sol}
    ...
    \end{sol}
    \item For each of the following pairs of integers $a$ and $n$, show that $a$ is relatively prime to $n$ and determine the multiplicative inverse of $\bar a$ in $\intmod$.
    \begin{problems}
        \item $a = 13, n = 20$
        \item $a = 69, n = 89$
        \item $a = 1891, n = 3797$
    ...
    \end{problems}
    \begin{solalph}
        \item Refer to the previous set of exercises on how to do the Euclidean Algorithm and how to calculate such an $x$ such that $ax + by = 1$, or $ax \equiv 1 \bmod n$. For this exercise, we obtain $-3 \equiv 17 \bmod 20$, so the inverse is $\bar{17}$.
        \item $40(69) - 31(89) = 1 \implies 40\cdot 89 \equiv 1 \bmod 89$, so the inverse is $\bar{40}$.
    ...
    \begin{breakablealgorithm}
        \begin{algorithmic}[1]
            ...
            \Ensure $\bar a + \bar b$, $\bar a \cdot \bar b$ (least residues modulo $n$), and optionally $\bar a^{-1}$ if $(a,n) = 1$

            \State \textbf{Function} \textsc{ModAdd}($a,b,n$)
            \State Sum $\gets a + b$
            \State \textbf{While} Sum $\ge n$ \textbf{Do}
                \State Sum $\gets Sum - n$
            \State \textbf{While} Sum $< 0$ \textbf{Do}
                \State Sum $\gets Sum + n$
            \State \textbf{Return} Sum

            \State \textbf{Function} \textsc{ModMul}($a,b,n$)
            \State Prod $\gets 0$
            \State A\_Mod $\gets a$
            \State \textbf{While} A\_Mod $\ge n$ \textbf{Do}
                \State A\_Mod $\gets A\_Mod - n$
            \State \textbf{For} i $= 1$ \textbf{To} b \textbf{Do}
                \State Prod $\gets \textsc{ModAdd}(Prod, A\_Mod, n)$
            \State \textbf{Return} Prod

            \State \textbf{Function} \textsc{ModInverse}($a,n$)
            \State \textbf{If} GCD$(a,n) \neq 1$ \textbf{Then Return} ``No Inverse''
            \State \textbf{For} c $= 1$ \textbf{To} $n-1$ \textbf{Do}
                \State \textbf{If} $\textsc{ModMul}(a,c,n) = 1$ \textbf{Then Return} c
            \State \textbf{Return} ``No Inverse Found''

            \State \textbf{Example Usage:}
            \State Input: $a=7, b=5, n=12$
            \State Output: $\textsc{ModAdd}(7,5,12) = 0$, $\textsc{ModMul}(7,5,12) = 11$
            \State If Requested: $\textsc{ModInverse}(7,12) = 7$
        \end{algorithmic}
    \end{breakablealgorithm}
    \end{sol}
\end{problems}