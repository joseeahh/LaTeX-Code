...
\begin{problems}
    ...
    \end{solalph}
    \item \label{ex3.1.26} Let $a,b \in G$.
    \begin{problems}
    ...
    \end{sol}
    \item \label{ex3.1.34} Let $D_{2n} = \langle r,s \mid r^n = s^2 = 1,\ rs = sr^{-1} \rangle$ be the usual presentation of the dihedral group of order $2n$ and let $k$ be a positive integer dividing $n$.
    \begin{problems}
    ...
    \begin{sol}
        We know that $\speclin_n(F) \leq \gl_n(F)$, so we just need to show that $A \speclin_n(F) A\inv = \speclin_n(F)$ for all $A \in \gl_n(F)$. To that end, note that for any $S \in \speclin_n(F)$ and $X \in \gl_n(F)$, then we have $\det(XSX\inv) = \det(X)\det(S)\det(X\inv) = 1$, hence $X\speclin_n(F)X\inv \subseteq \speclin_n(F)$, and $\speclin_n(F) \nsub \gl_n(F)$.
...
\begin{problems}
    ...
    \end{problems}
    \begin{solalph}
        \item Consider the $(p - 1)$-tuples of elements of $G$, and call this set $\ss'$. Clearly, $\ss'$ has $|G|^{p-1}$ elements. Define the map $\phi : \ss' \to \ss$ by $\phi(x_1, x_2, \dots, x_{p-1}) = (x_1, x_2, \dots, x_{p-1}, (x_1x_2 \cdots x_{p-1})\inv)$. Now suppose $\phi(x_1, \dots, x_{p-1}) = \phi(y_1, \dots, y_{p-1})$. Then we have $x_i = y_i$ for all $1 \leq i \leq p - 1$, and the last inverse term must imply $x_1x_2 \cdots x_{p - 1} = y_1y_2 \cdots y_{p - 1}$ since inverses are unique in a group. Hence $\phi$ is injective. Moreover, this mapping is clearly surjective. It follows that $\abs\ss = \abs{\ss'} = |G|^{p-1}$.
...
\begin{problems}
    ...
    \begin{sol}
        ...
        \end{enumerate}
    \end{sol}
    \item \label{ex3.3.3} Prove that if $H$ is a normal subgroup of $G$ of prime index $p$ then for all $K \leq G$ either
    \begin{enumerate}
        \item[(i)] $K \leq H$ or
...
\begin{problems}
    ...
    \end{sol}
    \item Prove the following special case of part (2) of the Jordan--H\"older Theorem: assume the finite group $G$ has two composition series
    ...
    \begin{sol}
        ...
        Noting that $M_1 \nsub G$ and $N_{r - 1} \nsub G$, we may use the Second Isomorphism Theorem to conclude that $N_{r - 1}M_1/M_1 \cong N_{r - 1}/(N_{r - 1} \cap M_1) = N_{r - 1}/1 \cong N_{r - 1}$. Observe now that $M_1 \leq N_{r - 1}M_1 \leq G$, hence $N_{r - 1}M_1/M_1 \nsub G/M_1$. By simplicity of $G/M_1$, it must be that $N_{r - 1}M_1 = 1$ or $N_{r - 1}M_1 = G$. Since $N_{r - 1}$ is not trivial, it must be that $N_{r - 1}M_1 = G$. Now, $N_{r - 1}$ is simple, hence $N_{r - 2} = 1 = N_0$, and we may conclude that $r = 2$. Moreover, the composition factors are isomorphic:
        \[G/N_{r - 1} \cong M_1/(N_{r - 1} \cap M_1) \cong M_1 \quad \text{and} \quad N_{r - 1}/1 \cong N_{r - 1} \cong G/M_1 \qh\]
    \end{sol}
    \item Prove part (2) of the Jordan--H\"older Theorem by induction on $\min\{r,s\}$. [Apply the inductive hypothesis to $H = N_{r-1} \cap M_{s-1}$ and use the preceding exercises.]
    \begin{sol}
    ...
    \end{sol}
    \item Prove that if $H$ is a nontrivial normal subgroup of the solvable group $G$ then there is a nontrivial subgroup $A$ of $H$ with $A \trianglelefteq G$ and $A$ abelian.
    \begin{sol}
...
\begin{problems}
    \item In \hyperref[ex1.3.1]{Exercise 1.3.1} and \hyperref[ex1.3.2]{Exercise 1.3.2} you were asked to find the cycle decomposition of some permutations. Write each of these permutations as a product of transpositions. Determine which of these is an even permutation and which is an odd permutation.
    \begin{sol}
    ...
    \end{sol}
    \item Prove that $S_n$ is generated by $\{(i\ i+1)\mid 1 \le i \le n-1\}$. [Consider conjugates, viz.\ $(2\ 3)(1\ 2)(2\ 3)^{-1}$.]
    \begin{sol}
    ...
    \end{sol}
    \item Show that $S_n=\langle (1\ 2),(1\ 2\ 3 \ldots n)\rangle$ for all $n\ge2$.
    \begin{sol}
    ...
    \end{sol}
    \item Show that if $p$ is prime, $S_p=\langle \sigma,\tau\rangle$ where $\sigma$ is any transposition and $\tau$ is any $p$-cycle.
    \begin{sol}
    ...
    \end{sol}
    \item Show that $\langle (1\ 3),(1\ 2\ 3\ 4)\rangle$ is a proper subgroup of $S_4$. What is the isomorphism type of this subgroup?
    \begin{sol}
    ...
    \end{sol}
    \item Prove that the group of rigid motions of a tetrahedron is isomorphic to $A_4$. [Recall \hyperref[ex1.7.20]{Exercise 1.7.20}.]
    \begin{sol}
    ...
    \end{sol}
    \item \label{ex3.5.8} Prove the lattice of subgroups of $A_4$ given in the text is correct. (By the preceding exercise and the comments following Lagrange's Theorem, $A_4$ has no subgroup of order $6$.)
    \begin{sol}
    ...
    \end{sol}
    \item \label{ex3.5.9} Prove that the (unique) subgroup of order $4$ in $A_4$ is normal and is isomorphic to $V_4$.
    \begin{sol}
    ...
    \end{sol}
    \item Prove that $A_n$ contains a subgroup isomorphic to $S_{n-2}$ for each $n\ge3$.
    \begin{sol}
    ...
    \end{sol}
    \item Prove that every element of order $2$ in $A_n$ is the square of an element of order $4$ in $S_n$. [An element of order $2$ in $A_n$ is a product of $2k$ commuting transpositions.]
    \begin{sol}
    ...
    \end{sol}
    \item Let $x$ and $y$ be distinct $3$-cycles in $S_5$ with $x\ne y^{-1}$.
    \begin{problems}
    ...
    \end{solalph}
    \item If $x$ and $y$ are $3$-cycles in $S_n$, prove that $\langle x,y\rangle$ is isomorphic to $\mathbb{Z}_3$, $A_4$, $A_5$, or $\mathbb{Z}_3\times\mathbb{Z}_3$.
    \begin{sol}
