...
\item Prove that if $G$ is an abelian simple group then $G \cong \mathbb{Z}_p$ for some prime $p$
(do not assume $G$ is a finite group).

\item Exhibit all $3$ composition series for $Q_8$ and all $7$ composition series for $D_8$.
List the composition factors in each case.

\item Find a composition series for the quasidihedral group of order $16$
(cf.\ Exercise~11, Section~2.5). Deduce that $QD_{16}$ is solvable.

\item Use Cauchy's Theorem and induction to show that a finite abelian group has a subgroup
of order $n$ for each positive divisor $n$ of its order.
...
\item If $G$ is a finite group and $H \trianglelefteq G$, prove that there is a composition series of $G$,
one of whose terms is $H$.

\item Let $G$ be a finite group. Prove that the following are equivalent:
\begin{enumerate}
...
\noindent
[For (iv), prove that a minimal nontrivial normal subgroup $M$ of $G$ is necessarily abelian
and then use induction. To see that $M$ is abelian, let $N \trianglelefteq M$ be of prime index
(by (iii)) and show that $x^{-1}y^{-1}xy \in N$ for all $x,y \in M$
(cf.\ Exercise~40, Section~1). Apply the same argument to $gNg^{-1}$ to show that
$x^{-1}y^{-1}xy$ lies in the intersection of all $G$-conjugates of $N$, and use the minimality
of $M$ to conclude that $x^{-1}y^{-1}xy = 1$.]
