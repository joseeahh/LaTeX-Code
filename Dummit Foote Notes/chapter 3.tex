...
\begin{theo}[The Fourth/Lattice Isomorphism Theorem]
    ...
    \end{enumerate}
    \begin{proof}
        Let $\ac = \set{A \leq G \mid N \subseteq A}$ be the set of subgroups of $G$ containing $N$, and let $\bar\ac = \set{\bar A \mid A \in \ac}$. Define the map $\phi : \ac \to \bar\ac$ by $\phi(A) = A/N$. To show that $\phi$ is a bijection, we first show it is surjective. Let $\bar A \in \bar\ac$, so that $\bar A = B/N$ for some $B \in \ac$. Then $\phi(B) = B/N = \bar A$, hence $\phi$ is surjective. To show injectivity, suppose $\phi(A_1) = \phi(A_2)$ for $A_1, A_2 \in \ac$. Then $A_1/N = A_2/N$, so that for any $a_1 \in A_1$, there exists $a_2 \in A_2$ such that $a_1N = a_2N$, hence $a_2\inv a_1 \in N \subseteq A_2$ so that $a_1 \in A_2$. By symmetry, then $A_1 = A_2$, hence $\phi$ is injective. We now prove the properties:
        \begin{enumerate}
            \item If $A \leq B$, then for any $aN \in \bar A$, we have $a \in A \leq B$ so that $aN \in \bar B$, hence $\bar A \leq \bar B$. Conversely, if $\bar A \leq \bar B$, then for any $a \in A$, we have $aN \in \bar A \leq \bar B$ so that $aN = bN$ for some $b \in B$. It follows that $b\inv a \in N \subseteq B$, hence $a \in B$, so that $A \leq B$.
            \item If $A \leq B$, consider the map $\psi : B/A \to \bar B/\bar A$ given by $\psi(bA) = bN \bar A$. To show $\psi$ is well-defined, suppose $b_1A = b_2A$ for $b_1, b_2 \in B$. Then $b_2\inv b_1 \in A$, so that $b_2\inv b_1N \in \bar A$, hence $b_1N \bar A = b_2N \bar A$. It is clear that $\psi$ is a homomorphism and surjective. Finally, if $bA \in \ker\psi$, then $bN \bar A = \bar A$ so that $b \in A$, hence $bA = A$. It follows that $\psi$ is an isomorphism, so that $|B : A| = |\bar B : \bar A|$.
            \item Let $x \in \bar{\gen{A, B}}$, so that $x = gN$ for some $g \in \gen{A, B}$. Then $g$ may be written as a finite product of elements of $A$ and $B$, so that $x$ may be written as a finite product of elements of $\bar A$ and $\bar B$, hence $x \in \gen{\bar A, \bar B}$. Conversely, let $y \in \gen{\bar A, \bar B}$, so that $y = gN$ for some $g \in G$ that may be written as a finite product of elements of $A$ and $B$. It follows that $g \in \gen{A, B}$, hence $y \in \bar{\gen{A, B}}$. Therefore, the two sets are equal.
            \item Pick $x \in \bar{A \cap B}$ so that $x = gN$ for some $g \in A \cap B$. It follows that $g \in A$ and $g \in B$, hence $gN \in \bar A$ and $gN \in \bar B$, so that $x \in \bar A \cap \bar B$. Conversely, let $y \in \bar A \cap \bar B$, so that $y = gN$ for some $g \in G$ such that $gN \in \bar A$ and $gN \in \bar B$. It follows that $g \in A$ and $g \in B$, hence $g \in A \cap B$, so that $y \in \bar{A \cap B}$. Therefore, the two sets are equal.
            \item \rightimp Suppose $A \nsub G$. Suppose $gN \in \bar G$ and consider $(gN)(aN)(g\inv N) \in gN(\bar A)g\inv N$ for some $aN \in \bar A$. Note that $(gN)(aN)(g\inv N) = gag\inv N$. Since $A \nsub G$, then $gag\inv \in A$, hence $gag\inv N \in \bar A$, so that $gN \bar A g\inv N \subseteq \bar A$. By \autoref{theo3.6}, then $\bar A \nsub \bar G$.
            
            \noindent \leftimp Suppose $\bar A \nsub \bar G$. To show that $A \nsub G$, let $g \in G$ and consider $gag\inv$ for $a \in A$. Note that $gN \in \bar G$ and $gN \bar A g\inv N \in gN \bar A g\inv N$ is equal to $gag\inv N$. Since $\bar A \nsub \bar G$, then $gag\inv N \in \bar A$, hence $gag\inv \in A$, so that $gAg\inv \subseteq A$. By \autoref{theo3.6}, then $A \nsub G$. \qh
        \end{enumerate}
    \end{proof}
\end{theo}
